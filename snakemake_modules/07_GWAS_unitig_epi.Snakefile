...
metadata_folder="snakemake_modules/gwas_metadata_sepidermidis/" 
output_folder="data/genomes_analysis/GWAS/sepidermidis/GWAS_final_paper_sepi_2022_fnalmetdata_updatedtree/"
tree="/data/sahebkashafs2/AD_clean/data/genomes_analysis/tree/sepidermidis_GCF_000007645.1_ASM764v1_genomic_paperfinal/results_paperfinal/gubbins/gubbins.final_tree.tre"
cpus=40
GWAS_dir="data/genomes_analysis/GWAS_epi_2022/"
dimensions="4"

phenos_ratio=["SAP_vs_all_ratio","SAP_vs_SEP_ratio"]
...
#copy reference
rule copy_ref:
  input:"data/genomes/reference/sepidermidis/GCF_000007645.1_ASM764v1_genomic.fa"
  output:
...
rule make_ref_multiple:
   ...
   cut -f1 {input.strains} | grep -v 'ID' > {params.strains}
...
rule annotate_multiple:
    input:
        unitig_filtered=output_folder+"gwas_unitig_{pheno_column}/unitig_significance_filtered.txt",
        unitig=output_folder+"gwas_unitig_{pheno_column}/unitig_significance.txt",
        reference=output_folder+"reference/references_multiple.txt"
    output:
...
rule summarise:
    input: output_folder+"gwas_unitig_{pheno_column}/unitig_significance_annotated_filtered.txt",
