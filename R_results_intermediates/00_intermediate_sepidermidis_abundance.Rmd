...
subset to metagenomes that we have cultured from
...
#melt
relabund=melt(data.matrix(relabund))
colnames(relabund)=c("Identifier","Sample_ID","relab")
relabund=relabund[which(relabund$Identifier=="SMGC_241"),] 
...
# format deffile 
deffile=read.delim("results/strains/abundance/sepidermidis/seq_deffile.txt", stringsAsFactors = F, sep="\t")
...
extra=data.frame(filename=c("adtrc","adtqs"),ST=c("16_epi","5_epi"))
...
abundance=read.delim("results/strains/abundance/sepidermidis/abundance_all_vf.tsv", stringsAsFactors = F, header=F)
...
saveRDS(abundance_Ac,"intermediates/strain_abundance_sepidermidis_Ac_BIB.RDS")


#merge BIB with relative abundance
...
saveRDS(abundance,"intermediates/strain_abundance_sepidermidis_BIB.RDS")
